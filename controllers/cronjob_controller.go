...
import (
	...
	batchv1 "test-kubebuilder/api/v1"
)
...
func (r *CronJobReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	for index, job := range childJobs.Items {
		_, finishedType := isJobFinished(&job)
		switch finishedType {
		case "":
			activeJobs = append(activeJobs, &childJobs.Items[index])
		case kbatch.JobFailed:
			failedJobs = append(failedJobs, &childJobs.Items[index])
		case kbatch.JobComplete:
			successfulJobs = append(successfulJobs, &childJobs.Items[index])
		}
	...
	// Check if the cronJob was suspended
